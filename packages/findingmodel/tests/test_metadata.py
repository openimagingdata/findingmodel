"""Tests for canonical structured metadata types."""
...
def test_subspecialty_from_canonical_values() -> None:
    ...
    assert Subspecialty("NM") == Subspecialty.NM
    assert Subspecialty("NR") == Subspecialty.NR
    assert Subspecialty("SQ") == Subspecialty.SQ
    assert Subspecialty("VA") == Subspecialty.VA
    assert Subspecialty("MK") == Subspecialty.MK


def test_subspecialty_all_members() -> None:
    expected = {"BR", "CA", "CH", "ER", "GI", "GU", "HN", "IR", "MI", "MK", "NM", "NR", "OB", "OI", "PD", "SQ", "VA"}
    actual = {member.value for member in Subspecialty}
    assert actual == expected


def test_subspecialty_meaning_property() -> None:
...
def test_finding_model_full_json_roundtrip_with_new_fields() -> None:
    ...
    model = _make_full_model(
        ...
        subspecialties=[Subspecialty.GU, Subspecialty.GI],
    ...
    assert loaded.subspecialties == [Subspecialty.GU, Subspecialty.GI]
...
def test_format_age_profile_all_ages() -> None:
    from findingmodel import format_age_profile
...
def test_format_time_course_duration_only() -> None:
    from findingmodel import format_time_course
