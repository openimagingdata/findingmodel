...
# Build-time dependencies (openai) are imported lazily in functions that need them
# to avoid requiring openai at runtime for query/stats operations
...
def _get_location_columns(dimensions: int = 1536) -> dict[str, str]:
...
def _create_schema(conn: duckdb.DuckDBPyConnection, dimensions: int = 1536) -> None:
