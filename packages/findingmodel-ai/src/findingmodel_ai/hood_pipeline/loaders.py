...
def should_process_file(file_path: Path, all_files: List[Path]) -> bool:
    ...
    if file_path.suffix == ".json":
        # Skip .cde.json files; process other JSON (they take priority over MD)
        return not file_path.name.endswith(".cde.json")

    if file_path.suffix == ".md":
        json_path = file_path.with_suffix(".json")
        return json_path not in all_files

    return False


async def load_definition(file_path: Path) -> Tuple[Optional[Dict], Optional[str], str]:  # noqa: RUF029
