...
CLI entry: ``single-agent-pipeline`` (``findingmodel_ai.single_agent_pipeline:run``),
formerly ``findingmodels/scripts/single_agent_pipeline.py``.
...
or merges with existing ones. Uses a single GPT agent with tools.
"""
...
import findingmodel_ai.compat  # noqa: F401 - documented hook for findingmodel / findingmodel-ai
from findingmodel_ai.agents.single_agent import AgentContext, ProcessingResult, create_single_agent
...
def _get_agent() -> Agent[AgentContext, ProcessingResult]:
...
async def generate_sub_finding_models(
    ...
    for name in findings_to_create:
        ...
        try:
            ...
            tags = [*(base_stub.tags or []), f"sub_finding_of:{parent_name}"]
...
async def process_single_file(
    ...
    try:
        ...
        content = (
            json.dumps(data, indent=2, ensure_ascii=False) if file_type == "json" else (markdown_content or "")
        )
...
def generate_attribute_report(  # noqa: C901
    ...
    files_with_renamed = sum(
        1
        for _, _, info in attribute_tracking
        if info
        and (
            info.get("presence", {}).get("action") in ["renamed", "classification_used"]
            or info.get("change_from_prior", {}).get("action") in ["renamed", "classification_used"]
        )
    )
    files_with_added = sum(
        1
        for _, _, info in attribute_tracking
        if info
        and (
            info.get("presence", {}).get("action") == "added"
            or info.get("change_from_prior", {}).get("action") == "added"
        )
    )
    ...
    # Presence statistics
    presence_exact = sum(
        1 for _, _, info in attribute_tracking if info and info.get("presence", {}).get("action") == "exact_match"
    )
    presence_renamed_heuristic = sum(
        1 for _, _, info in attribute_tracking if info and info.get("presence", {}).get("action") == "renamed"
    )
    presence_renamed_classification = sum(
        1
        for _, _, info in attribute_tracking
        if info and info.get("presence", {}).get("action") == "classification_used"
    )
    presence_added = sum(
        1 for _, _, info in attribute_tracking if info and info.get("presence", {}).get("action") == "added"
    )
    ...
    change_renamed_heuristic = sum(
        1
        for _, _, info in attribute_tracking
        if info and info.get("change_from_prior", {}).get("action") == "renamed"
    )
    change_renamed_classification = sum(
        1
        for _, _, info in attribute_tracking
    ...
    for filename, finding_name, tracking_info in sorted(attribute_tracking, key=lambda x: x[1].lower()):
        ...
        if merge_info:
            if merge_info.get("match_found"):
                ...
                lines.append(
                    f"- **Database match:** Found - `{existing.get('name', 'unknown')}` (ID: {existing.get('oifm_id', 'N/A')})"
                )
                    ...
                    if merge_details:
                        ...
                        if merge_details.get("merge_recommendations"):
                            ...
                            for mr in merge_details["merge_recommendations"]:
                                lines.append(
                                    f"    - `{mr.get('existing_name')}` ← `{mr.get('incoming_name')}` ({mr.get('relationship')})"
                                )
                        if merge_details.get("new_attributes"):
                            lines.append(
                                f"  - New attributes added: {', '.join([f'`{n}`' for n in merge_details['new_attributes']])}"
                            )
                else:
        ...
        if missing_attrs_for_file and missing_attrs_for_file[0]:
            lines.append(
                f"- **Missing attributes from source:** {', '.join(f'`{attr}`' for attr in missing_attrs_for_file[0])}"
            )
...
def generate_sub_finding_report(  # noqa: C901
    ...
    files_with_extractions = sum(1 for _, _, info in sub_finding_tracking if info and info.get("extracted"))
    files_with_kept = sum(1 for _, _, info in sub_finding_tracking if info and info.get("kept_with_presence"))
...
async def process_hood_directory(  # noqa: C901
    input_dir: str,
    output_dir: str,
    ...
    files_to_process = [f for f in all_files if should_process_file(f, all_files)]
    ...
    for file_path in files_to_process:
        success, message, tracking_info, sub_finding_info, merge_tracking_info, missing_attrs = (
            await process_single_file(file_path, index, output_path)
        )
        ...
        if success:
            ...
            if tracking_info:
                ...
                finding_name = tracking_info.get("presence", {}).get("finding_name") or tracking_info.get(
                    "change_from_prior", {}
                ).get("finding_name", "unknown")
    ...
    if attribute_tracking:
        report_path = generate_attribute_report(
            attribute_tracking, merge_tracking, missing_attributes_tracking, input_dir, str(output_path)
        )
...
async def main() -> None:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Import Hood CT Chest definitions from CDEStaging repository")
    parser.add_argument(
        "--log-level",
...
def run() -> None:
    """Sync entrypoint for ``python -m findingmodel_ai.single_agent_pipeline`` (and ``if __name__ == "__main__"``)."""
    asyncio.run(main())
