...
async def enrich_finding_unified(  # noqa: C901
    ...
    async def generate_ontology_queries() -> list[str]:
        ...
        try:
            result: list[str] = await generate_finding_query_terms(finding_name, description)
            return result
        except Exception as e:
    ...
    # Search anatomic locations via AnatomicLocationIndex
    ...
    try:
        ...
        async with AnatomicLocationIndex() as index:
            ...
            anatomic_results = await execute_anatomic_search(query_info, index, limit=30)
    ...
    from findingmodel.protocols import TABLE_TO_INDEX_CODE_SYSTEM
    ...
    try:
        ...
        async with fm_index:
            ...
            if entry is not None:
                existing_model = await fm_index.get_full(entry.oifm_id)
