...
def test_bioontology_client_no_api_key() -> None:
    ...
    # Temporarily clear the API key if it exists (uses findingmodel_ai.config.settings)
    original_key = getattr(ai_config.settings, "bioontology_api_key", None)
    try:
        if original_key:
            ai_config.settings.bioontology_api_key = None
        with pytest.raises(ValueError, match="API key is required"):
            BioOntologySearchClient(api_key=None)
    finally:
        if original_key:
            ai_config.settings.bioontology_api_key = original_key
