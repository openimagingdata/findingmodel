...
- Always use the Serena MCP server for repository context and note-taking. Prefer Serena tools for symbol-aware navigation (`find_symbol`, `get_symbols_overview`, `find_referencing_symbols`), and use `search_for_pattern` for plain-text matching.
- Run lookups with Serena tools before scanning files manually.
